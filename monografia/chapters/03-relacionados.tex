...
criados para domínios variados. No domínio de EDA, o repositório
\texttt{KiCAD-MCP-Server} \citep{kicadmcpserver} consolidou uma implementação
de referência em Python e TypeScript que serve de base ao presente trabalho:
o servidor analisado neste texto é um \emph{fork} mantido no workspace do
autor, que preserva a arquitetura de duas camadas e incorpora correções e
extensões próprias, documentadas em capítulos posteriores.
...
\toprule
\textbf{Critério} & \textbf{\emph{Scripts} e plugins} & \textbf{Modelo adaptado ao domínio} & \textbf{Servidor MCP (este trabalho)} \\
\midrule
...
projeto real. O presente trabalho endereça essa lacuna construindo e
avaliando o \texttt{mcp-kicad-vscode} com esse tripé.
