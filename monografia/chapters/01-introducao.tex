...
projeto; de outro, avaliar uma base de integração aberta que possa ser
inspecionada, testada e estendida pela comunidade.
...
Analisar e avaliar o \texttt{mcp-kicad-vscode} --- o servidor \emph{Model
Context Protocol} de código aberto do projeto \texttt{KiCAD-MCP-Server}, criado
por \emph{mixelpixx}~\citep{kicadmcpserver} ---, que expõe o fluxo de projeto
de placas de circuito impresso do KiCad a assistentes de inteligência
artificial, e conduzir um estudo de caso com uma placa de quatro camadas
projetada com o auxílio do servidor, com ênfase em cobertura funcional,
mecanismos de descoberta, integrações de fabricação e verificação automatizada.

\subsection{Objetivos específicos}

\begin{enumerate}[leftmargin=1.4em,itemsep=0.2em]
  \item analisar a arquitetura do servidor e o protocolo interno entre as
        camadas de integração;
  \item caracterizar o catálogo de ferramentas MCP cobrindo esquemático,
        layout, roteamento, regras de projeto, bibliotecas, exportação e
        cadeia de suprimentos;
  \item avaliar os mecanismos de descoberta de ferramentas e de exposição de
        estado do projeto como recursos MCP;
  \item executar e analisar as suítes de teste e a integração contínua do
        projeto, verificando o comportamento do servidor;
  \item avaliar o servidor por meio de métricas de implementação e de um
        estudo de caso com uma placa real de quatro camadas projetada com o
        auxílio do servidor;
...
A pesquisa é aplicada, de natureza tecnológica, com análise de um artefato de
\emph{software} existente e avaliação por estudo de caso instrumental. As
etapas compreendem auditoria do domínio e do código, definição de requisitos
de avaliação, análise da arquitetura em duas camadas, verificação por testes e
integração contínua, e estudo de caso com uma placa seguidora de linhas
projetada com o auxílio do servidor. O Capítulo~\ref{cap:metodologia} detalha
o percurso e as métricas adotadas.
