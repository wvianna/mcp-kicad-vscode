...
ferramentas e fontes de dados externas. Este trabalho analisa e avalia o
\texttt{mcp-kicad-vscode}, o servidor MCP de código aberto do projeto
\texttt{KiCAD-MCP-Server}, criado por \emph{mixelpixx}, dedicado à automação
do fluxo de projeto de placas de circuito impresso (PCB) no KiCad. O servidor
...
ignorados na execução local de setembro de 2026 --- e um estudo de caso,
conduzido pelo autor, com uma placa seguidora de linhas de quatro camadas (97
\emph{footprints}, 83 redes, aproximadamente 82,1\,mm $\times$ 80,2\,mm), cuja
verificação de regras reportou 11 violações residuais (9 erros e 2 avisos) e
nenhum item não conectado. Discutem-se as limitações --- integração IPC não exercitada no
...
and data sources. This work analyzes and evaluates \texttt{mcp-kicad-vscode},
the open-source MCP server from the \texttt{KiCAD-MCP-Server} project, created
by \emph{mixelpixx}, dedicated to automating the printed
circuit board (PCB) design workflow in KiCad. The server adopts a two-layer
...
case study, conducted by the author, with a four-layer line-follower board
(97 footprints, 83 nets, approximately 82.1\,mm $\times$ 80.2\,mm), whose design rule check reported 11
